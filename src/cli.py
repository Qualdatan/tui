...
def interactive_setup() -> dict:
    ...
    print_header("Simple BIM Pipeline", "Qualitative Analyse -- Neue Analyse")
    ...
    if not transcripts:
        ...
        console.print("  Bitte lege deine Transkripte dort ab und starte erneut.")
        sys.exit(1)

    console.print(f"\n[dim]Gefundene Transkripte in {TRANSCRIPTS_DIR}[/dim]")
    selected_transcripts = _pick_multiple(
        "Welche Transkripte sollen analysiert werden?",
        transcripts,
    )
    print_success(f"{len(selected_transcripts)} Transkript(e) ausgewaehlt")

    # 2. Methode (Recipe) auswaehlen
    recipes = list_recipes()
    if not recipes:
        print_error("Keine Methoden gefunden. Bitte pruefe den Ordner 'methods/' (oder METHODS_DIR in .env).")
        sys.exit(1)
    ...
    print_success(f"Methode: {recipe_id}")
    ...
    if codebases:
        ...
        print_warning("Keine Codebasen in input/codebases/ gefunden.")
        console.print("  [dim]Codes werden automatisch abgeleitet (induktiv)[/dim]")

    console.print()
    console.print(
        Panel(
            "[bold green]Konfiguration abgeschlossen -- starte Analyse...[/bold green]",
            box=box.ROUNDED,
            border_style="green",
            padding=(0, 2),
        )
